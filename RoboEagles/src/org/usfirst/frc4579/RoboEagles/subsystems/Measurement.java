...
public class Measurement extends Subsystem {
    ...
    public void measure() {
    	...
    	acceleration.X = accelerationAverageX.filter((MPU_accelerometer.getAccelX()) - accelOffset.X) * GsToAccel; // UNTESTED LINE!
    	acceleration.Y = accelerationAverageY.filter((MPU_accelerometer.getAccelY()) - accelOffset.Y) * GsToAccel; // UNTESTED LINE!
    	...
    	double accelMagnitude = distance.magnitude();
    	
    	distance.X += accelMagnitude * Math.sin(robotAngle * DegreesToRadians);
    	distance.Y += accelMagnitude * Math.cos(robotAngle * DegreesToRadians);
    	
    	//	Could we monitor Z acceleration to reduce noise? 
